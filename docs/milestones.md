...
- [x] Image target: `us-central1-docker.pkg.dev/<PROJECT_ID>/docker-repo/diagram-architect-api`.
...
- [x] `curl https://<CLOUD_RUN_URL>/api/v1/health` returns `status: UP`.
- [x] `curl https://<CLOUD_RUN_URL>/api/v1/diagrams/types` returns the diagram types list.
